    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'glass border-b border-white/5' : ''}`}>
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <a href={buildPath(lang, 'demo')} onClick={(e) => handleNav(e, 'demo')} className="px-5 py-2 rounded-lg bg-gradient-to-r from-violet-500 to-blue-500 hover:from-violet-400 hover:to-blue-400 text-sm font-medium text-white transition-colors animate-pulse">Hackathon Demo</a>
        </div>
      ...
      {mobileOpen && (
        <div className="md:hidden glass border-t border-white/5 px-6 py-4 space-y-3">
          ...
          <a href={buildPath(lang, 'demo')} onClick={(e) => handleNav(e, 'demo')} className="block px-4 py-2 rounded-lg bg-gradient-to-r from-violet-500 to-blue-500 text-sm font-medium text-white text-center animate-pulse">Hackathon Demo</a>
        </div>
      )}
    </nav>
  )
