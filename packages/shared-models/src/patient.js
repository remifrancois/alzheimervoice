...
import { readSecureJSON, writeSecureJSON } from './secure-fs.js';

const DATA_DIR = path.resolve(process.env.DATA_ROOT || './data', 'patients');
...
export async function listPatients() {
  ...
  for (const file of files) {
    if (file.endsWith('.json') && !file.startsWith('memories_')) {
