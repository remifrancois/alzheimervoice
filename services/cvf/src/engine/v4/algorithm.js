/**
 * V4 CORE SCORING ALGORITHM — 9-Domain Mixed Text+Audio
 *
 * Extends V3's deterministic scoring to 85 indicators across 9 domains.
 * When audio is unavailable, weights redistribute so V4 text-only >= V3.
 *
 * Pipeline:
 *   transcript + audio → 85 features → z-scores → 9-domain scores
 *   → confounder adjustment → composite → 3 cascade detectors
 *   → sentinel check → decline profile → alert level
 *
 ...
 */
export function computeV4Baseline(sessionVectors, minSessions = 14) {
...
/**
 * Compute per-domain scores from z-scores. Covers all 9 domains.
 */
export function computeDomainScores(zScores) {
  ...
  for (const [domain, indicatorIds] of Object.entries(DOMAINS)) {
    ...
    for (const id of indicatorIds) {
      const z = zScores[id];
      if (z != null) valid.push({ z, weight: INDICATORS[id].base_weight || 0.5 });
    }
...
/**
 * 9-domain composite. When acoustic/pd_motor have null scores,
...
// ════════════════════════════════════════════════
// CASCADE DETECTION (3 cascade types)
// ════════════════════════════════════════════════
...
export function detectCascade(domainScores) {
  ...
  return {
    alzheimer: ad, parkinson: pd, depression: dep,
    all: [...ad, ...pd, ...dep].sort((a, b) => b.severity - a.severity)
  };
}
...
/** PD Cascade — NEW. Monopitch → phonatory → articulatory → prosodic collapse. */
...
/** Depression Cascade — NEW. Affective shift → temporal retardation → engagement withdrawal. */
...
/** Check V4 expanded sentinels: 7 AD, 7 depression, 10 PD. */
...
export function computeDeclineProfile(history) {
  ...
  if (pdC >= 1 && adC <= 1 && depC === 0) profileType = 'pd_like';
...
/**
 * Run the complete V4 analysis pipeline on a session.
 * 1. Z-scores → 2. Domain scores (9) → 3. Confounders → 4. Composite
 * → 5. Alert level → 6. Cascade (AD/PD/Dep) → 7. Sentinels → 8. Decline profile
 */
export function analyzeSession(sessionVector, baseline, confounders = {}, history = []) {
  ...
  return {
    version: 'v4', composite, alert_level: alertLevel,
...
/** Run weekly aggregation over sessions. Same V3 pattern, 9 domains. */
export function analyzeWeek(sessions, baseline, weekNumber) {
  if (sessions.length === 0) return null;

  const results = sessions.map(s => analyzeSession(s.feature_vector, baseline, s.confounders));
  ...
  return {
    version: 'v4', week_number: weekNumber, composite,
